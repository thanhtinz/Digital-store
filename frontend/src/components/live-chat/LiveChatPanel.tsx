...
// chat như các app bán hàng, KHÔNG điều hướng sang trang /chat riêng.
// Dùng API: GET /api/chat/my (tải), POST /api/chat/send (gửi), GET /api/chat/poll
...
export default function LiveChatPanel() {
  ...
  // Tải hội thoại khi mở khung lần đầu.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        // Nếu khung đang đóng và có tin từ admin -> tăng badge chưa đọc.
        if (!open) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Khôi phục nội dung nếu gửi lỗi để người dùng thử lại.
      setText(body);
    } finally {
      setSending(false);
    }
  ...
  // Nút nổi (đóng) — đặt cao hơn thanh điều hướng dưới trên mobile.
  if (!open) {
    return (
      <Fab
        color="primary"
    ...
    <Slide direction="up" in={open} mountOnEnter unmountOnExit>
      <Paper
        ...
        sx={{
          ...
          // Mobile: bottom-sheet gần như toàn màn hình. Desktop: thẻ nổi góc phải.
          bottom: { xs: 0, lg: 24 },
          right: { xs: 0, lg: 24 },
          left: { xs: 0, lg: 'auto' },
          top: { xs: 0, lg: 'auto' },
          width: { xs: '100%', lg: 380 },
          height: { xs: '100%', lg: 560 },
          maxHeight: { lg: '80vh' },
