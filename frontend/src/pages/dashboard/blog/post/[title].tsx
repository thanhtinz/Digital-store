...
export default function BlogPostPage() {
  ...
  const getPost = useCallback(async () => {
    try {
      const response = await axios.get(`/api/blog/posts/${title}`);
  ...
  const getRecentPosts = useCallback(async () => {
    try {
      ...
      setRecentPosts(response.data.posts || response.data.recentPosts || []);
    } catch (error) {
      console.error(error);
    }
  }, [title]);
  ...
  return (
    <>
      <Head>
        <title>{`${post?.title || 'Blog'} | Digital Store`}</title>
      </Head>
